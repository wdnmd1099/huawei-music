...
})({"kk65R":[function(require,module,exports) {
...
class Player {
    ...
    start() {
        ...
        fetch("https://karr.top/music-data/music-data/music-data.json").then((res)=>res.json()).then((data)=>{
            console.log(data);
            this.songList = data;
    ...
    bind() {
        ...
        this.$(".btn-play-pause").onclick = function() {
            if (this.classList.contains("playing")) self.pauseSong();
            else if (this.classList.contains("pause")) self.playSong();
        };
        this.$(".btn-pre").onclick = function() {
            if (self.currentIndex <= self.songList.length - 1 && self.currentIndex > 0) self.audio.src = self.songList[self.currentIndex -= 1].url;
            else {
        ...
        this.$(".btn-next").onclick = function() {
            if (self.currentIndex < self.songList.length - 1) self.audio.src = self.songList[self.currentIndex += 1].url;
            else {
                self.currentIndex = 0;
                self.audio.src = self.songList[self.currentIndex].url;
            }
    ...
    getNews() {
        ...
        .then((res)=>res.text()).then((data)=>{
            //console.log(data)
            test = data.split(/\s/).filter((str)=>str.match(/\[.+?\]/)).forEach((X)=>{
                ci = X.replace(/\[.+]/gm, "") //筛选出歌词
                ;
                X.match(/\[.+?\]/g) //删选出时间戳，并转化为毫秒
                .forEach((t)=>{
                    t = t.replace(/[\[\]]/g, "");
                    time = parseInt(t.slice(0, 2)) * 60000 + parseInt(t.slice(3, 5)) * 1000 + parseInt(t.slice(6)) //转成毫秒数
                    ;
                    ZZZ.push([
                        time,
                        ci
...
},{"./icons.js":"kUw3l"}],"kUw3l":[function(require,module,exports) {
...
},{}]},["kk65R","dP9tu"], "dP9tu", "parcelRequire5c03")
